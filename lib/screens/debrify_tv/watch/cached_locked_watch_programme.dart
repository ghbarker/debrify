...
enum CachedLockedProvider { realDebrid, allDebrid }

Future<void> runCachedLockedWatch(
  WatchFlowBindings host,
  List<Torrent> cachedTorrents, {
  required CachedLockedProvider provider,
  ...
  final apiKey = await (provider == CachedLockedProvider.realDebrid
  ...
  if (apiKey == null || apiKey.isEmpty) {
    ...
    MainPageBridge.notifyAutoLaunchFailed(
      provider == CachedLockedProvider.realDebrid
          ? 'No Real Debrid API key'
          : 'No AllDebrid API key',
    );
    host.showSnack(
      provider == CachedLockedProvider.realDebrid
  ...
  if (provider == CachedLockedProvider.realDebrid &&
      host.rdSkipBlockedTorrents) {
  ...
  try {
    ...
    debugPrint(
      provider == CachedLockedProvider.realDebrid
          ? 'DebrifyTV: Cached watch flow finished.'
          : 'DebrifyTV: AllDebrid cached watch flow finished.',
    );
  }
}
