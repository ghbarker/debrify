...
import 'dialogs/community_channels_dialog.dart';
import 'dialogs/export_channels_dialog.dart';
...
/// Screen-owned import/export orchestration relocated from the M1-2 service.
/// Seam is [ChannelImportExportHost] / [ProgressSink] + `channels`
...
class ChannelImportExport {
  ...
  Future<bool> importTextBytes(String sourceName, Uint8List bytes) async {
    final keywords = parseChannelText(bytes);
  ...
  Future<String> generateChannelYaml(DebrifyTvChannel channel) async {
    ...
    return serializeChannelYaml(channel, cachedTorrents);
  }
