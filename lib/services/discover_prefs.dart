...
class DiscoverPrefs {
  ...
  // Replaces the old separate discover_show_ratings/discover_show_titles
  // keys AND Home's home_hide_card_titles_and_ratings — see Settings ›
  // Layout › Title & Ratings Visibility. AppMigrationService merges any
  // pre-existing values into this key once per profile.
  static const String _titleRatingsVisibleKey =
  ...
  static bool _titleRatingsVisible = true;
  ...
  static Future<void> warmUp() async {
    ...
    try {
      ...
      _titleRatingsVisible = prefs.getBool(_titleRatingsVisibleKey) ?? true;
    } catch (_) {
  ...
  /// Whether posters — on Discover's grid, a Home row expansion, or Home's
  /// own cards — show their title and rating. Unset is on, so existing
  /// profiles keep their current presentation until they explicitly hide it.
  /// Replaces the old separate Discover showRatings/showTitles and Home
  /// hideCardTitlesAndRatings settings (see [_titleRatingsVisibleKey]).
  static bool get titleRatingsVisible => _titleRatingsVisible;
  ...
  static Future<void> setTitleRatingsVisible(bool value) async {
    _titleRatingsVisible = value;
    try {
      final prefs = await ProfilePreferences.instance();
      await prefs.setBool(_titleRatingsVisibleKey, value);
    } catch (_) {
      // Best-effort: the choice still holds for this session.
    }
  }
