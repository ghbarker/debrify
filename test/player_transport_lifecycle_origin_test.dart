...
class _Properties implements mk.NativePlayer {
  ...
  void emitPlayback(Duration position, {bool playing = false}) {
...
void main() {
  ...
  tearDown(() async {
    try {
      ...
      debugPrintSynchronously('TRANSPORT_TEARDOWN $error\n$stack');
  ...
  Future<void> withHost(
    ...
    try {
      ...
      primaryStack = stack;
      debugPrintSynchronously('TRANSPORT_PRIMARY $error\n$stack');
      rethrow;
    } finally {
      try {
        ...
        debugPrintSynchronously('TRANSPORT_CLEANUP $error\n$stack');
        if (primaryFailure != null) {
          Error.throwWithStackTrace(primaryFailure!, primaryStack!);
        }
        rethrow;
  ...
  void expectVisible(WidgetTester tester, bool visible) {
    final control = find.byType(TvControls);
    final opacity = find.ancestor(
      of: control,
      matching: find.byType(AnimatedOpacity),
    );
    expect(opacity, findsOneWidget);
    final opacityWidget = tester.widget<AnimatedOpacity>(opacity);
    expect(opacityWidget.child, isA<IgnorePointer>());
    final pointer = opacityWidget.child! as IgnorePointer;
    expect(pointer.child, isA<ExcludeFocus>());
    final focus = pointer.child! as ExcludeFocus;
    expect(find.byWidget(pointer), findsOneWidget);
    expect(find.byWidget(focus), findsOneWidget);
    expect(
      find.descendant(of: find.byWidget(focus), matching: control),
      findsOneWidget,
    );
    expect(opacityWidget.opacity, visible ? 1 : 0);
    expect(pointer.ignoring, !visible);
    expect(focus.excluding, !visible);
  }

  testWidgets(
    'TV paused auto-hide re-arms a fresh interval and returns borrowed focus',
    (tester) async {
      await withHost(tester, (backend) async {
        final playPause = controls(tester).playPauseFocusNode;
        final bar = controls(tester).scopeNode;
        final progress = controls(tester).progressFocusNode;
        expect(controls(tester).isPlaying, isFalse);
        await tester.sendKeyEvent(LogicalKeyboardKey.escape);
        await tester.pump();
        expectVisible(tester, false);
        final root = FocusManager.instance.primaryFocus;
        expect(root, isNotNull);
        expect(root!.debugLabel, 'tvPlayerRoot');
        await tester.sendKeyEvent(LogicalKeyboardKey.arrowDown);
        await tester.pump();
        await tester.pump();
        expectVisible(tester, true);
        expect(FocusManager.instance.primaryFocus, same(playPause));

        // t=3: pause blocks and re-arms. At t=6, clearing the prior blocker buys
        // a complete fresh interval, so hiding cannot occur until t=9.
        await tester.pump(const Duration(seconds: 3));
        expectVisible(tester, true);
        backend.emitPlayback(const Duration(seconds: 2), playing: true);
        await tester.pump();
        expect(controls(tester).isPlaying, isTrue);
        await tester.pump(const Duration(seconds: 3));
        expectVisible(tester, true);
        ...
        expect(controls(tester).playPauseFocusNode, same(playPause));
        expect(controls(tester).scopeNode, same(bar));
  ...
  testWidgets(
    'TV nested menu BACK returns to rail before closing without popping player',
    (tester) async {
      await withHost(tester, (backend) async {
        await tester.sendKeyEvent(LogicalKeyboardKey.escape);
        await tester.pump();
        ...
        final playPause = controls(tester).playPauseFocusNode;
        expect(FocusManager.instance.primaryFocus, same(playPause));
        controls(tester).onSpeed();
        await tester.pump();
        await tester.pump(const Duration(milliseconds: 320));
        expect(find.byType(PlayerMenuPanel), findsOneWidget);
        expectVisible(tester, false);
        final panelFocus = FocusManager.instance.primaryFocus;
        expect(panelFocus!.debugLabel, 'player-menu-panel');
        await tester.sendKeyEvent(LogicalKeyboardKey.arrowRight);
        await tester.pump();
        await tester.sendKeyEvent(LogicalKeyboardKey.escape);
        await tester.pump();
        // A failed pane entry would close immediately and fail this assertion.
        expect(find.byType(PlayerMenuPanel), findsOneWidget);
        expect(FocusManager.instance.primaryFocus, same(panelFocus));
        expect(observer.pops, 0);
        await tester.sendKeyEvent(LogicalKeyboardKey.escape);
        await tester.pump();
