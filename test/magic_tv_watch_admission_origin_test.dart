...
// M1-7 admission origin: 1b5eeb83be007615bcfbda48e616f186910504d7.
...
void main() {
  ...
  tearDown(() async {
    ...
    if (!provider.firstLocked.isCompleted) provider.firstLocked.complete(null);
  ...
  for (final id in [CloudProviderId.premiumize, CloudProviderId.alldebrid]) {
    for (final phase in ['alpha-first', 'beta-first', 'cancel-held-engine']) {
      testWidgets(
        'origin ${id.name} admission two-engine $phase preserves duplicate quality and preparation timing',
        (tester) async {
          ...
          const unfiltered = bool.fromEnvironment('ADMISSION_UNFILTERED');
          ...
          addTearDown(() {
            ...
            if (!unfiltered) expect(diagnostics, [_editorPaintDiagnostic]);
          });
          ...
          await http.runWithClient(
            () async {
              ...
              expect(provider.checkCacheCount, 0);
              ...
              if (phase == 'cancel-held-engine') {
                if (id == CloudProviderId.premiumize) {
                  await _until(tester, () => provider.checkCacheCount == 1);
                  expect(provider.lastCacheQuery!.toSet(), {
                    _hash(1),
                    _hash(3),
                  });
                }
                ...
                provider.firstLocked.complete(null);
                await _until(
                  tester,
                ...
                if (id == CloudProviderId.premiumize) {
                  expect(provider.checkCacheCount, 1);
                  expect(provider.lastCacheQuery!.toSet(), {
                    _hash(1),
                    _hash(3),
                  });
