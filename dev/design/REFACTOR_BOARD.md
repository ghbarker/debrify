...
|---|---|---|---|---|---|---|
| C0 · CI truth | review | `refactor/c0-ci-truth` | worker-c0 | `.github/workflows/test.yml`, `tool/ci_test_allowlist.py`, `tool/ci_test_allowlist_test.py`, `test/BASELINE_ALLOWLIST.txt`, `test/BASELINE_FAILURES.md`, `dart_test.yaml`, new `tool/check_layering.dart`, new `tool/analyze_baseline.json`, plus a new layering test file if needed | — | Analyze-all fails on a new diagnostic in `search_screen.dart` without committing it. Goldens: Linux tolerance job, goldens not deleted. |
