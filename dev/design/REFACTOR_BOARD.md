...
| G1'-3 · keyword search | review | `refactor/g1p-3-keyword-search` | worker | `search_screen.dart` [keyword hunks], `lib/services/search/keyword_search_controller.dart`, `lib/screens/search/keyword_search_screen.dart`, pin | G1'-2 | Leaves −2 379. Pin predates move. Host keeps thin `_switchMode` launcher + `_openKeywordBind`. |
...
| V1-4 · IPTV recording | review | `refactor/v1-4-iptv-recording` | worker | `video_player_screen.dart` [recording hunks], `lib/services/playback/iptv_recording_controller.dart` | V1-3 | Leaves 704. Pin predates move. Desktop capture `extension: 'ts'`; Android engine-first then tee. Overlay reads notifiers via host getters. |
...
| M1-1 · channel cache warmer | review | `refactor/m1-1-channel-cache-warmer` | worker | `magic_tv_screen.dart` [warm/cache hunks], `lib/services/debrify_tv/channel_cache_warmer.dart` | M1-0 | **#87 resubmit.** Leaves **856** (host 10 764 → 9 908) meets 850. TorBox window, quality-torrent filter, parseKeywords, save-warm/prune, RD size-filter compute moved. Create/update dialogs stay (M1-5). Pin predates move. |
