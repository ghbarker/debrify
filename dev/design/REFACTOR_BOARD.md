...
| G4 · step 2 remaining hosts | review | `refactor/g4-cloud-files-rest` | worker | `premiumize/**`, `alldebrid/**`, `pikpak/**` files screens, `CloudFilesScreen` | — | Route PM/AD/PikPak onto shared `CloudFilesScreen`. Selection bar stays on hosts (shape-manifest). Public types and sidebar ids frozen. |
