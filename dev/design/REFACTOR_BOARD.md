...
| G3 · storage split | assigned | `refactor/g3-storage-split` | worker | `storage_service.dart`, `lib/services/storage/**`, storage key-sweep tests | — | First slice: key ownership + one domain store with forwarding façade. Keys/encodings frozen. Gate (c). Rebase after G4 if branched before this merge (CODEMAP/BOARD only). |
