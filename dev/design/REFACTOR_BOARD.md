...
| G4 · cloud file screens | review | `refactor/g4-cloud-files-screen` | worker | `debrid_downloads_screen.dart`, `torbox/**`, `premiumize/**`, `alldebrid/**`, `pikpak/**` files screens | — | Shared `CloudFilesScreen` + local `CloudFilesSource`. **This PR: RD + TorBox on the shared screen.** Premiumize/AllDebrid/PikPak left on their own screens (follow-up). Supersedes closed #36–#43. Gate (c). |
