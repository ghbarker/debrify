...
| P2c · launcher + bulk-add | review | `refactor/p2c-launcher-bulk-add` | worker-p2c | `lib/services/video_player_launcher.dart`, `lib/services/torrent_bulk_add_service.dart` | — | Forbidden: `lib/services/cloud/**`. No dart format on launcher. Frozen: resume prefixes, analytics `real_debrid`, dialog `realdebrid`. Did not edit the shared fake (subclassed in the pin test). |
