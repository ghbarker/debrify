...
| G1 · step 2 CatalogSearchController | review | `refactor/g1-catalog-search-controller` | worker | `lib/screens/search_screen.dart` [catalog search hunks], `lib/screens/search/**`, new catalog search controller + tests | G1 step 1 | Extract catalog search only. Pin + origin-diff. `search_screen.dart` 18 890 → 18 814. Do not start steps 3–5. Gate (c). |
