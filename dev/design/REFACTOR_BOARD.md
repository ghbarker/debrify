...
| G1 · step 5 TV stages | review | `refactor/g1-tv-stages` | worker | `lib/screens/search_screen.dart` [six TV stage layouts], `lib/screens/search/search_stage_widgets.dart`, new `lib/screens/search/stages/**` | G1 step 4 | Pin + origin-diff. `search_screen.dart` 18 580 → 17 039. Empty Spotlight still `break`s to classic. `_buildDiscoverStage` left on host (Discover chrome). |
