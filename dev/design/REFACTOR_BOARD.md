...
| P2b · Stremio TV strings | review | `refactor/p2b-stremio-tv-strings` | worker-p2b | `lib/screens/stremio_tv/**` | — | Forbidden: `lib/services/cloud/**`. Cache-check overlay and debug label go through `StremioTvDispatch` (`is` + FakeCloudProvider `supports` fallback). Frozen: picker ids (`realdebrid`), `auto` sentinel. |
