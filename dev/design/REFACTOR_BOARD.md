...
| G3 · step 2 remaining Home keys | review | `refactor/g3-home-prefs-rest` | worker | `storage_service.dart`, `lib/services/storage/home_prefs.dart`, key-sweep tests | G3 slice 1 | Remaining Home keys in HomePrefs. `storage_service.dart` 9 835 → 9 634. Callers stay on StorageService. Gate (c) pin+origin-diff. `clearAllHomePageSettings` still skips Trakt defaults (NOTES). PlayerPrefs not started. `@Deprecated` in Q2. |
