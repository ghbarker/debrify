...
Baseline: `main` @ G3 merged · Phase: **2** (G5 + G1 step 4 assigned) · Last gate: 1 (Linux + Windows build/smoke **pass**; Android not run)
...
| G5 · scrobble coordinator | review | `refactor/g5-scrobble-coordinator` | worker | `video_player_screen.dart` [scrobble hunks], `services/*/*_scrobble_session.dart`, new coordinator + tests | — | Unify duplicated Trakt/Simkl machines. Keep Simkl pause-centric (no `/scrobble/start`). Gate (c). `video_player_screen.dart` 16 278 → 15 771. |
