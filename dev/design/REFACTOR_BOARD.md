...
Baseline: `main` @ G4 merged · Phase: **2** (G1 step 3 + G3 + G5 assigned) · Last gate: 1 (Linux + Windows build/smoke **pass**; Android not run)
...
| G1 · step 3 TitleOpener | assigned | `refactor/g1-title-opener` | worker | `lib/screens/search_screen.dart` [`_openItem` + menu building], `lib/screens/search/title_opener.dart`, tests | G1 step 2 | Extract `_openItem` and menu building only. Do not start steps 4–5. Gate (c). Rebase after G4 if branched before this merge (CODEMAP/BOARD only). |
...
| G3 · storage split | review | `refactor/g3-storage-split` | worker | `storage_service.dart`, `lib/services/storage/**`, storage key-sweep tests | — | First slice: key ownership pin + **HomePrefs** (page defaults). `storage_service.dart` 9 963 → 9 835. Callers still on StorageService. Next slice: remaining Home keys (disabled/extra/order/hero/ticks). No `@Deprecated` (would warn call sites). Gate (c). |
...
| G5 · scrobble coordinator | assigned | `refactor/g5-scrobble-coordinator` | worker | `video_player_screen.dart` [scrobble hunks], `services/*/*_scrobble_session.dart`, new coordinator + tests | — | Unify duplicated Trakt/Simkl machines. Keep Simkl pause-centric (no `/scrobble/start`). Gate (c). Rebase after G4 if branched before this merge (CODEMAP/BOARD only). |
...
Phase 1 merged. P2a–**P2e** merged (#60). **G1 step 1 merged (#61).** **T2 merged (#62).** **G2 merged (#63).** **G4 merged (#64).** **G1 step 2 merged (#65).** G1 step 3 + G3 + G5 assigned (rebase onto this `main` if they branched earlier — CODEMAP/BOARD only). Gate 1 Windows pass; Android not run. PR #56 held. #36–#43 closed (G4).
