...
| P1b · RD/AD Magic-TV port | review | `refactor/p1b-magic-tv-unlock` | worker | `lib/services/cloud/**`, `test/cloud_*` | — | **#76.** Thin `CloudMagicTvRdUnlock` / `CloudMagicTvAdUnlock`. `magic_tv_screen.dart` untouched. Unblocks M1. |
...
| G1'-1 · catalog play resolver | assigned | `refactor/g1p-1-catalog-play-resolver` | worker | `search_screen.dart` [play/resume hunks], `lib/services/playback/catalog_play_resolver.dart` | G1'-0 | Extract `_onCatalogPlay` resolve half. Leaves ~850. No new `part`/`extension on` State. |
...
| V1-1 · resume controller | assigned | `refactor/v1-1-resume-controller` | worker | `video_player_screen.dart` [resume hunks], `lib/services/playback/resume_controller.dart` | V1-0 | Leaves ~650. Pure; 0 setState. |
