...
| G5 · scrobble coordinator | review | `refactor/g5-scrobble-coordinator` | worker | `video_player_screen.dart` [scrobble hunks], `services/*/*_scrobble_session.dart`, new coordinator + tests | — | Unify duplicated Trakt/Simkl machines. Keep Simkl pause-centric (no `/scrobble/start`). Gate (c). `video_player_screen.dart` 16 278 → 15 756. |
