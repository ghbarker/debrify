...
Baseline: `main` @ #73–#81 · Phase: **2 correction** (S2-2 + G1'-2 + V1-2 review) · Last gate: 1 (Linux + Windows build/smoke **pass**; Android not run)
...
| G1'-2 · source edit/add dialogs | review | `refactor/g1p-2-source-binding-dialogs` | worker | `search_screen.dart` [source dialog hunks], `lib/widgets/sources/source_binding_dialogs.dart`, pin + widget tests | G1'-1 | **#84.** Leaves −486. `onReorder` ignore (path-keyed baseline). Pin predates move. |
...
| M1-0 · WatchSession | merged | `refactor/m1-0-watch-session` | worker | `magic_tv_screen.dart` [WatchSession field/sink hunks], `lib/screens/debrify_tv/watch_session.dart`, tests | P1b merged (#76) | **#81.** Leaves +12 ≈ 0. `ProgressSink` seam. Pin predates move. Watch flows / import / cache warmer not extracted. |
| M1-1 … M1-6 | queued | — | — | `magic_tv_screen.dart` | M1-0 | Sequential after M1-0. See PHASE2 §3. Target ≤ 4 500. Do not start M1-1 until assigned. |
...
Phase 1 merged. Old Phase 2 G1–G5/T2/G3-HomePrefs **closed as insufficient** (audit in PHASE2 §1). Binding is **#72** `REFACTOR_PLAN_PHASE2.md`. **G3 PlayerPrefs parked** (`refactor/g3-player-prefs` — do not merge; S2-3 later). **G5 closed** (remainder is V1-9). **#76–#80 merged** (P1b, S2-1, G1'-1, V1-1, G4 rest). Open: **#81 M1-0**, **#82 S2-2**. Mini-gate after every three merged extractions per lane. PR #56 held for Q3. #36–#43 closed (G4).
